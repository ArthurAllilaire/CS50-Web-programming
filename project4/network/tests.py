...
class UserModelTests(TestCase):
    ...
    def test_user_add_following(self):
        ...
        self.assertQuerysetEqual(
            user.follows.all(), User.objects.filter(username="test_user1"))
    ...
    def test_user_adds_followers(self):
        ...
        # user should now have queryset with user1
        self.assertQuerysetEqual(user1.followers.all(), User.objects.exclude(
            username="test_user1"), ordered=False)
...
class PostModelTests(TestCase):
    ...
    def test_liking_post(self):
        ...
        self.assertQuerysetEqual(
            post3.likes.all(), User.objects.exclude(username=user3), ordered=False)
    ...
    def test_unliking_post(self):
        ...
        self.assertQuerysetEqual(post3.likes.all(), User.objects.exclude(
            username=user3).exclude(username=user), ordered=False)
...
class PostsViews(TestCase):
    def test_following_posts(self):
        ...
        self.assertQuerysetEqual(
            response.context["posts"], Post.objects.all().order_by("-date"))
        ...
        # Make sure the text is contained on the page
        self.assertContains(
            response, "Post1" and "Post2" and "Post3" and "Post4")
        ...
        # Make sure posts are in the context and in the right order
        self.assertQuerysetEqual(
            response.context["posts"], Post.objects.filter(user=user2).order_by("-date"))

        # Make sure the text is contained on the page
        self.assertContains(response, "Post3" and "Post4")


""" Client side testing using web browser simulation """
